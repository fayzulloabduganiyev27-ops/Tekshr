...
# --- Do'kon: buyurtmalar ---
async def create_shop_order(user_id: int, product_id: int, product_name: str, price_som: int) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "INSERT INTO shop_orders (user_id, product_id, product_name, price_som) VALUES (?, ?, ?, ?)",
            (user_id, product_id, product_name, price_som),
        )
        await db.commit()
        return cur.lastrowid
...
async def create_topup_request(user_id: int, amount_som: int) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "INSERT INTO topup_requests (user_id, amount_som) VALUES (?, ?)", (user_id, amount_som)
        )
        await db.commit()
        return cur.lastrowid
...
async def shop_product_callback(callback: CallbackQuery) -> None:
    ...
    order_id = await create_shop_order(callback.from_user.id, product_id, name, price_som)
...
async def approve_shop_order(callback: CallbackQuery) -> None:
    ...
    if not order or order[5] != "pending":
        ...
        return
    _, user_id, product_id, product_name, price_som, status, _ = order
    user = await get_user(user_id)
    balance_som = user[8] if user and len(user) > 8 else 0
    if not user or balance_som < price_som:
        await callback.answer("Foydalanuvchi balansi yetarli emas!", show_alert=True)
...
async def topup_amount_received(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    request_id = await create_topup_request(message.from_user.id, amount)
    await message.answer(
...
async def approve_topup(callback: CallbackQuery) -> None:
    ...
    if not request or request[3] != "pending":
        await callback.answer("Bu so'rov allaqachon ko'rib chiqilgan.", show_alert=True)
        return
    _, user_id, amount_som, status, _ = request
