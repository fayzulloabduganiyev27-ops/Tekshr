...
async def init_db() -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cur_ch_cols = await db.execute("PRAGMA table_info(channels)")
        ch_columns = [row[1] for row in await cur_ch_cols.fetchall()]
        if "invite_link" not in ch_columns:
            await db.execute("ALTER TABLE channels ADD COLUMN invite_link TEXT")
        if "limit_count" not in ch_columns:
            await db.execute("ALTER TABLE channels ADD COLUMN limit_count INTEGER DEFAULT 0")
        await db.execute(
...
async def get_channel_meta(channel: str):
    """(invite_link, limit_count) qaytaradi."""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT invite_link, limit_count FROM channels WHERE channel = ?", (channel,)
        )
        row = await cur.fetchone()
...
async def record_join_request(user_id: int, channel: str) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR IGNORE INTO join_requests (user_id, channel) VALUES (?, ?)", (user_id, channel)
        )
        await db.commit()
...
async def has_join_request(user_id: int, channel: str) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT 1 FROM join_requests WHERE user_id = ? AND channel = ?", (user_id, channel)
        )
...
def _channel_api_id(channel: str):
    """@username bo'lsa o'zini, raqamli ID (yopiq kanal) bo'lsa int qaytaradi."""
...
async def check_all_subscriptions(user_id: int) -> bool:
    ...
    for ch in channels:
        ...
        try:
            ...
            # Ba'zan Telegram xato qaytaradi (masalan foydalanuvchi hech qachon
            # a'zo bo'lmagan bo'lsa) — bu holatda ham pastdagi zayavka tekshiruviga o'tamiz,
            # darhol False qaytarmaymiz.
            is_member = False
        ...
        # a'zo emas — lekin qo'shilish uchun so'rov (zayavka) yuborgan bo'lishi mumkin
...
async def maybe_auto_remove_channel(channel: str) -> None:
    """Kanalga belgilangan sondagi kishi qo'shilsa, uni majburiy ro'yxatdan avtomatik olib tashlaydi."""
...
async def track_join_request(request: ChatJoinRequest) -> None:
    """Kanal 'so'rov orqali qo'shilish' (zayavka) rejimida bo'lsa,
    bot so'rovni AVTOMATIK TASDIQLAMAYDI — faqat kim so'rov yuborganini qayd etadi.
    Shu qayd orqali obuna tekshiruvida zayavka yuborgan foydalanuvchi ham hisobga olinadi."""
    channels = await get_channels()
...
async def withdraw_username_received(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        f"🕵️ Sovg'a anonim tarzda yuboriladi (kimligingiz oshkor qilinmaydi)\n\n"
...
async def approve_shop_order(callback: CallbackQuery) -> None:
    ...
    if payment_channel:
        if anonymous:
            display_name = "Anonim foydalanuvchi"
        else:
        ...
        with suppress(Exception):
            await bot.send_message(
                payment_channel,
                f"🛍️ <b>{display_name}</b> muvaffaqiyatli <b>{product_name}</b> ({price_som} so'm) sotib oldi!",
            )
...
def _extract_forwarded_chat_id(message: Message) -> int | None:
    """Eski (forward_from_chat) va yangi (forward_origin) Bot API sxemalarini ikkalasini ham qo'llab-quvvatlaydi."""
