...
def admin_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="👥 Foydalanuvchilarni boshqarish", callback_data="ap_users")],
...
async def start_handler(message: Message) -> None:
    ...
    if existing and existing[7] == 1:  # banned
...
@dp.callback_query(F.data == "ap_users")
async def ap_users_prompt(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback.from_user.id):
        return
...
def user_actions_keyboard(user_id: int, banned: bool) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [ban_button],
            [InlineKeyboardButton(text="⬅️ Ortga", callback_data="ap_back")],
        ]
    )
...
async def ap_users_search(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    if not user:
        await message.answer("Bunday foydalanuvchi bazada topilmadi.", reply_markup=admin_panel_keyboard())
        return
...
async def ap_adjust_stars_save(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        f"✅ {target_id} uchun balans yangilandi: <b>{new_balance}</b> ⭐",
